...
use super::font::DebugScreenFont;
...
/// A debug screen writing directly in the VRAM.
///
/// Ported from the [`hello_world`] VitaSDK sample, without ANSI code support.
///
/// [`hello_world`](https://github.com/vitasdk/samples/tree/master/hello_world/)
pub struct DebugScreen<'a> {
    base: &'a mut [u32],
    font: DebugScreenFont,
    mutex: SceUID,
    coord_x: usize,
    coord_y: usize,
    // saved_x: usize,
...
impl<'a> Default for DebugScreen<'a> {
    fn default() -> Self {
...
impl<'a> DebugScreen<'a> {
    pub fn new(font: DebugScreenFont) -> Self {
        unsafe {
    ...
    fn tab_width(&self) -> usize {
        self.font.size_w * SCREEN_TAB_SIZE
    }

    fn puts(&mut self, text: &[u8]) {
        let bytes_per_glyph = self.font.width * self.font.height / 8;
        self.lock();

        for &chr in text.iter() {
            if chr == b'\t' {
                self.coord_x += self.tab_width() - (self.coord_x % self.tab_width());
                continue;
            }
            ...
            let mut font = &self.font.glyphs[(chr - self.font.first) as usize * bytes_per_glyph..];
            let mut mask = 1 << 7;
            ...
            for row in 0..self.font.height {
                ...
                for col in self.font.width..self.font.size_w {
                    vram[row * SCREEN_FB_WIDTH + col] = self.color_bg
                }
            }

            for row in self.font.height..self.font.size_h {
                for col in 0..self.font.size_w {
                    vram[row * SCREEN_FB_WIDTH + col] = self.color_bg
                }
            }

            self.coord_x += self.font.size_w;
        }
